...
#pragma managed
		...
		const unsigned char CommandSetDeviceMode2 = 0b11110011;  //dec: 243
		const unsigned char CommandSetDeviceMode2Demo = 0b11111111;  //dec: 255
		const GUID g_guidServiceClass = { 0x00001101, 0x0000, 0x1000,{ 0x80, 0x00, 0x00, 0x80, 0x5f, 0x9b, 0x34, 0xfb } };


		bool DataGeneratorOtto::IsFormeterAvailable(String^ RemoteName, bool ForceFlush)
		{
		...
		int DataGeneratorOtto::FindFormeterDevice(String^ RemoteName, SOCKADDR_BTH& RemoteBtAddr, bool ForceFlush, bool TestConnection)
		{
		...
		List<String^>^ DataGeneratorOtto::ListPairedDevices(String^ RemoteName, bool ForceFlush)
		{
		...
		DataGeneratorOtto::DataGeneratorOtto(Model model, Frequency SamplingFrequency) :
			...
			mDataReaderInterfaceHelper(NULL)
		{
			...
			AdapterOtto^ adapter = nullptr;
			if (model == OTComm::Otto::Models::OttoStandard)
				adapter = gcnew AdapterStandardOttoCh();
			else
				adapter = gcnew AdapterStandardQuattroCh;
		...
		void DataGeneratorOtto::SetDemoMode()
		{
			mDemoMode = true;
		}

		Bitmap^ DataGeneratorOtto::Image::get()
		{
			try
			{
		...
		DataGeneratorOtto^ DataGeneratorOtto::FromXMLConfiguration(String^ Filename)
		{
			DataGeneratorOtto^ gdr = nullptr;
		...
		AdapterOtto^ DataGeneratorOtto::GetAdapterFromID(String^ adapterID)
		{
			if (adapterID == ("StandardOttoAdapter8Ch"))
				return gcnew OTComm::Otto::AdapterStandardOttoCh();
			if (adapterID == ("StandardOttoAdapter4Ch"))
				return gcnew OTComm::Otto::AdapterStandardQuattroCh();
			return nullptr;
		}

		GenericChannel^ DataGeneratorOtto::GetChannelFromID(String^ channelID, String^ muscleDescription, String^ sideDescription)
		{
			return nullptr;
		}

		unsigned int DataGeneratorOtto::GetDeviceModel()
		{
			return (unsigned int)mModel;//TODO
		}

		unsigned int DataGeneratorOtto::GetNumberTotalChannels()
		{
			return mModel % 10000;
		}

		unsigned int DataGeneratorOtto::GetNumberNonAuxChannels()
		{
			return 0;
		}

		unsigned int DataGeneratorOtto::GetNumberAuxChannels()
		{
			return 0;
		}

		array<unsigned int>^ DataGeneratorOtto::CreateChannelsMapping()
		{
		...
		String^ DataGeneratorOtto::GetDeviceDescription(void)
		{
			String^ res = gcnew String("TRENTADUE");
			return res;
		}

		void DataGeneratorOtto::OpenDevice(String ^RemoteName)
		{
			LOG_ENTERING;
			try
			{
		...
		void DataGeneratorOtto::InitializeDevice()
		{
			LOG_ENTERING;
			try
			{
		...
		void DataGeneratorOtto::OuternThread()
		{
			LOG_ENTERING;
			try
			{
		...
		void DataGeneratorOtto::StartAcquisition(void)
		{
			LOG_ENTERING;
			try
			{
				...
				mManagedAcquisitionThread = gcnew Thread(gcnew ThreadStart(this, &DataGeneratorOtto::OuternThread));
		...
		void DataGeneratorOtto::StopAcquisition(void)
		{
			LOG_ENTERING;
			if ((SOCKET)mDeviceHandle == INVALID_SOCKET)
			{
		...
		void DataGeneratorOtto::CloseDevice(void)
		{
			LOG_ENTERING;
			if ((SOCKET)mDeviceHandle == INVALID_SOCKET)
			{
		...
		DataGeneratorOtto::~DataGeneratorOtto()
		{
			if (mDeviceIsOpen)
			{
				CloseDevice();
			}
			this->!DataGeneratorOtto();
		}
		...
		void DataGeneratorOtto::InternalStopFormeterDataTransfer(void)
		{
			LOG_ENTERING;
			int tmpResult = 0;
			if ((SOCKET)mDeviceHandle != INVALID_SOCKET)
		...
		void DataGeneratorOtto::ConnectIn(AdapterOtto^ adapter)
		{
			mConnectors[0]->Adapter = adapter;
		}

		int DataGeneratorOtto::ReadDeviceID(UCHAR* dev_id)
		{
			//TODO completare??????????????
		...
		void DataGeneratorOtto::CloseConnection()
		{
		...
		int DataGeneratorOtto::WriteByte(char Command)
		{
			if ((SOCKET)mDeviceHandle == INVALID_SOCKET)
			{
		...
		int DataGeneratorOtto::ReadBytes(char *ByteArray, int maxlen)
		{
			if ((SOCKET)mDeviceHandle == INVALID_SOCKET)
			{
				InvalidHandleException^ ihe = gcnew InvalidHandleException(__FUNCTION__);
		...
		int DataGeneratorOtto::GetFirmwareVersion(String^% version)
		{
		...
		int DataGeneratorOtto::GetBatteryLevel(String^% batterylevel)
		{
		...
		array<unsigned int>^ DataGeneratorOtto::GetAcquiredChannels()
		{
			return mAcquiredChannelsIndices;
		}


		String^ DataGeneratorOtto::GetDeviceXMLConfiguration()
		{
		...
		String^ DataGeneratorOtto::GetSavingXMLConfiguration()
		{
			OpenXMLConfigStream(false);
			//if (GetAcquiredChannels() == nullptr) return"";
			String^ Firmware;
		...
		String^ DataGeneratorOtto::UpdateFirmware(String^ filenamepath)
		{
		...
		ULONG DataGeneratorOtto::CreateSocketForDevice()
		{
...
#pragma unmanaged
		ULONG _stdcall AsyncIoOtto(PVOID  ThreadParameter)
		{
			const int NUM_DECODED_BUFFERS = 10;
			const int NUM_ASYNCH_IO = 1; //TODO controllare come mai non è 10
			...
			unsigned char Command = demomode ? CommandSetDeviceMode2Demo : CommandSetDeviceMode2;
			...
			WHILE(*mMustStopAcquisitionThread == false)
			{
				...
				if (ReceivedBufferIndex < NUM_ASYNCH_IO)
				{
					...
					for (unsigned int koffset = 0; koffset < numberOfBytesTransferred; koffset+=helper->mSource[0].mBytesPerChannel)
					{
						...
						else
						{
							...
							*DecodedBufferPos += (*AcquisitionBufferCurrentReadStart)<<8;
							AcquisitionBufferCurrentReadStart++;
							*/
							*DecodedBufferPos = *(AcquisitionBufferCurrentReadStart+1);
							*DecodedBufferPos <<= 8;
							*DecodedBufferPos += *AcquisitionBufferCurrentReadStart;
							AcquisitionBufferCurrentReadStart++;
						}
