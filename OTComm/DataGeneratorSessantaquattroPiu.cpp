#include "DataGeneratorSessantaquattroPiu.h"
#include "stdio.h"
#include <string>
...
#pragma region Constructors and destructors
		DataGeneratorSessantaquattroPiu::DataGeneratorSessantaquattroPiu(Model model, Frequency SamplingFrequency, bool HighRes) :
			...
			mDataReaderInterfaceHelper(NULL), mPortNumber(mDefaultPortNumber)
		{
			try
			{
				...
				mConnectors = gcnew array<GenericConnector^>(GetNumberConnectorsIn() + GetNumberConnectorsAux() + GetNumberQuaternion() + GetNumberControlAux());
				unsigned int k = 0;
				for (; k < GetNumberConnectorsIn(); k++)
					mConnectors[k] = gcnew GenericConnector(mModel % 10000);
				for (; k < GetNumberConnectorsIn()+GetNumberConnectorsAux(); k++)
					mConnectors[k] = gcnew GenericConnector(1);
				for (; k < GetNumberConnectorsIn() +GetNumberConnectorsAux() + GetNumberQuaternion(); k++)
					mConnectors[k] = gcnew GenericConnector(4, 0);
		...
		DataGeneratorSessantaquattroPiu::~DataGeneratorSessantaquattroPiu()
		{
			if (mDeviceIsOpen)
			{
				CloseDevice();
			}
			this->!DataGeneratorSessantaquattroPiu();
		}
...
#pragma region XML files
		...
		String^ DataGeneratorSessantaquattroPiu::GetDeviceXMLConfiguration()
		{
		...
		String^ DataGeneratorSessantaquattroPiu::GetSavingXMLConfiguration()
		{
			OpenXMLConfigStream(false);
			String^ Firmware;
			if (GetFirmwareVersion(Firmware) <= 0)
		...
		DataGeneratorSessantaquattroPiu^ DataGeneratorSessantaquattroPiu::FromXMLConfiguration(String^ Filename)
		{
			DataGeneratorSessantaquattroPiu^ gdr = nullptr;
			...
			try {
				...
				if (deviceName->Contains("SESSANTAQUATTRO+"))
				{
					...
					gdr = gcnew OTComm::SessantaquattroPiu::DataGeneratorSessantaquattroPiu(deviceModel, deviceFrequency,
						resolution == 3);
					...
					for (int ii = 0; ii < ChannelsNode->ChildNodes->Count; ii++)
					{
						...
						if (ad != nullptr)
						{
							...
							continue;
						if (adPosition < 1)
							gdr->ConnectIn(adPosition, (AdapterSessantaquattroPiu^)ad);
							...
							gdr->ConnectQuaternion(0, (AdapterQuaternions^)ad);
						}
						else // control
							gdr->ConnectControl(adPosition - 4, (AdapterControl^)ad);
...
#pragma region Connectors and adapters
		...
		void DataGeneratorSessantaquattroPiu::ConnectAux(const unsigned int port, AdapterAux^ adapter)
		{
		...
		void DataGeneratorSessantaquattroPiu::ConnectIsoAux(const unsigned int port, AdapterIsoAux^ adapter)
		{
			if (port >= GetNumberConnectorsAux())
			{
				InvalidConnectorNumberException^ icne = gcnew InvalidConnectorNumberException(__FUNCTION__);
		...
		void DataGeneratorSessantaquattroPiu::ConnectControl(const unsigned int port, AdapterControl^ adapter)
		{
			...
			mConnectors[GetNumberConnectorsIn()+GetNumberQuaternion() + GetNumberConnectorsAux() + port]->Adapter = adapter;
		}

		void DataGeneratorSessantaquattroPiu::ConnectQuaternion(const unsigned int port, AdapterQuaternions^ adapter)
		{
			if (port >= GetNumberQuaternion())
			{
				InvalidConnectorNumberException^ icne = gcnew InvalidConnectorNumberException(__FUNCTION__);
				FireOnError(icne);
				throw icne;
		...
		void DataGeneratorSessantaquattroPiu::ConnectIn(const unsigned int port, AdapterSessantaquattroPiu^ adapter)
		{
		...
		GenericAdapter^ DataGeneratorSessantaquattroPiu::GetAdapterFromID(String^ adapterID)
		{
			std::string converted = toStandardString(adapterID);


			if (adapterID == ("AD1x16SE"))
				return gcnew OTComm::SessantaquattroPiu::AdapterInAD1x16SE();
			if (adapterID == ("AD4x16SE"))
				return gcnew OTComm::SessantaquattroPiu::AdapterInAD4x16SE();
			if (adapterID == ("AD1x64SE"))
				return gcnew OTComm::SessantaquattroPiu::AdapterInAD1x64SE();
			if (adapterID == ("AD2x32SE"))
				return gcnew OTComm::SessantaquattroPiu::AdapterInAD2x32SE();
			if (adapterID == ("AD2x16SE"))
				return gcnew OTComm::SessantaquattroPiu::AdapterInAD2x16SE();
			if (adapterID == ("AD4x8SE"))
				return gcnew OTComm::SessantaquattroPiu::AdapterInAD4x8SE();
			if (adapterID == ("AD8x4SE"))
				return gcnew OTComm::SessantaquattroPiu::AdapterInAD8x4SE();
			if (adapterID == ("AD4x4SE"))
				return gcnew OTComm::SessantaquattroPiu::AdapterInAD4x4SE();
			if (adapterID == ("AD8x1SE"))
				return gcnew OTComm::SessantaquattroPiu::AdapterInAD8x1SE();
			if (StringContains("Direct connection to Auxiliary Input", converted))
				return gcnew OTComm::SessantaquattroPiu::AdapterAux();
			if (adapterID == ("ISO-AUX"))
				return gcnew OTComm::SessantaquattroPiu::AdapterIsoAux();
			if (adapterID == ("AdapterControl"))
				return gcnew OTComm::SessantaquattroPiu::AdapterControl();
			if (adapterID == ("ADEEGCap20SE"))
				return gcnew OTComm::SessantaquattroPiu::AdapterInADEEGCap20SE();
			if (adapterID == ("ADEEGCap64SE"))
				return gcnew OTComm::SessantaquattroPiu::AdapterInADEEGCap64SE();
			if (adapterID == ("AdapterQuaternions"))
				return gcnew OTComm::SessantaquattroPiu::AdapterQuaternions();
			return nullptr;
		}

		GenericChannel^ DataGeneratorSessantaquattroPiu::GetChannelFromID(String^ channelID, String^ muscleDescription, String^ sideDescription)
		{
			if (channelID == ("CoDe1"))
			...
			if (channelID == ("Quaternion"))
				return gcnew OTComm::Sensors::Quaternion(Muscles::ID(muscleDescription), MuscleSide::ID(sideDescription));
			return nullptr;
		}

		GenericChannel^ DataGeneratorSessantaquattroPiu::GetChannelFromString(String^ text)
		{
		...
		unsigned int DataGeneratorSessantaquattroPiu::GetNumberTotalChannels()
		{
			return GetNumberChannelsNonAux()+GetNumberChannelsQuaternion() + GetNumberChannelsAux() + GetNumberControlAux();
		}

		unsigned int DataGeneratorSessantaquattroPiu::GetNumberChannelsNonAux()
		{
			return (((unsigned int)mModel) % 10000);
		}

		unsigned int DataGeneratorSessantaquattroPiu::GetNumberChannelsQuaternion()
		{
			return 4;
		}

		unsigned int DataGeneratorSessantaquattroPiu::GetNumberChannelsAux()
		{
			return 2;
		}

		unsigned int DataGeneratorSessantaquattroPiu::GetNumberControlAux()
		{
			return 2;
		}

		unsigned int DataGeneratorSessantaquattroPiu::GetNumberQuaternion()
		{
			return 1;
		}

		unsigned int DataGeneratorSessantaquattroPiu::GetNumberConnectorsAux()
		{
			return GetNumberChannelsAux();
		}

		unsigned int DataGeneratorSessantaquattroPiu::GetNumberConnectorsIn()
		{
			return 1;
		}

		int DataGeneratorSessantaquattroPiu::PortNumber::get()
		{
			return mPortNumber;
		}
#pragma endregion
...
#pragma region Device properties 
		...
		void DataGeneratorSessantaquattroPiu::SetIPAddress(String^ IPAddress)
		{
			mIPAdress = IPAddress;
		}

		void DataGeneratorSessantaquattroPiu::SetIPPort(unsigned int IPPort)
		{
			mPortNumber = IPPort;
		}

		unsigned int DataGeneratorSessantaquattroPiu::GetDeviceModel()
		{
			return (unsigned int)mModel;
		}

		Bitmap^ DataGeneratorSessantaquattroPiu::Image::get()
		{
			return Utilities::GetImageFromEmbeddedResource("64_sessantaquattro.jpg", true);
		}

		String^ DataGeneratorSessantaquattroPiu::GetDeviceDescription(void)
		{
		...
		String^ DataGeneratorSessantaquattroPiu::IPAdress::get()
		{
...
#pragma region Public device operations
		void DataGeneratorSessantaquattroPiu::OpenDevice(void)
		{
		...
		void DataGeneratorSessantaquattroPiu::InitializeDevice()
		{
			LOG_ENTERING;
			try
			{
		...
		void DataGeneratorSessantaquattroPiu::StartAcquisition(void)
		{
			LOG_ENTERING;
			/*if ((SOCKET)mDeviceHandle == INVALID_SOCKET)
			{
			...
			mManagedAcquisitionThread = gcnew Thread(gcnew ThreadStart(this, &DataGeneratorSessantaquattroPiu::OuternThread));
			mManagedAcquisitionThread->Start();
			//}
			LOG_EXITING;
		}

		void DataGeneratorSessantaquattroPiu::StopAcquisition(void)
		{
			LOG_ENTERING;
			/*if ((SOCKET)mDeviceHandle != INVALID_SOCKET)
			{*/
		...
		void DataGeneratorSessantaquattroPiu::CloseDevice(void)
		{
			LOG_ENTERING;
			/*if ((SOCKET)mDeviceHandle != INVALID_SOCKET)
			{
...
#pragma region Non public device operations
		...
		int DataGeneratorSessantaquattroPiu::ReadDeviceID(UCHAR* dev_id)
		{
		...
		int DataGeneratorSessantaquattroPiu::GetFirmwareVersion(String^% version)
		{
			LOG_ENTERING;
			int ver = 0;
		...
		array<Byte>^ DataGeneratorSessantaquattroPiu::CreateStartCommand()
		{
		...
		array<Byte>^ DataGeneratorSessantaquattroPiu::CreateStopCommand()
		{
			array<Byte>^ command = CreateSetCommand();
			command[1] += 0;
			return command;
		...
		array<Byte>^ DataGeneratorSessantaquattroPiu::CreateSetCommand()
		{
			...
			{
				if (GetConnectors()[0]->DetectionMode == SessantaquattroPiu::DetectionModes::Bipolar)// bipolar
				{
		...
		array<Byte>^ DataGeneratorSessantaquattroPiu::CreateImpedenceCheckCommand()
		{
			array<Byte>^ command = gcnew array<Byte>(13);
		...
		array<Byte>^ DataGeneratorSessantaquattroPiu::CreateAdvancedImpedenceCheckCommand()
		{
			array<Byte>^ command = gcnew array<Byte>(13);
		...
		array<Byte>^ DataGeneratorSessantaquattroPiu::CreateGetCommand()
		{
			array<Byte>^ command = gcnew array<Byte>(13);
			command[0] += 128;
			return command;
		...
		Byte DataGeneratorSessantaquattroPiu::ComputeCRC(array<Byte>^ command, unsigned int Len)
		{
		...
		void DataGeneratorSessantaquattroPiu::OuternThread()
		{
			try
			{
				...
				HANDLE mAsyncIoThreadHandle = CreateThread(
					...
					AsyncIoSessantaquattroPiu,  // Thread Func
		...
		void DataGeneratorSessantaquattroPiu::InternalStopSessantaquattroPiuDataTransfer(void)
		{
			LOG_ENTERING;
			//if ((SOCKET)mDeviceHandle != INVALID_SOCKET)
			//{
		...
		void DataGeneratorSessantaquattroPiu::CloseConnection()
		{
		...
		int DataGeneratorSessantaquattroPiu::WriteString(String^ command)
		{
			LOG_ENTERING;
			if ((SOCKET)mDeviceHandle == INVALID_SOCKET)
			{
		...
		int DataGeneratorSessantaquattroPiu::ReadString(String^% read)
		{
			LOG_ENTERING;
			if ((SOCKET)mDeviceHandle == INVALID_SOCKET)
			{
...
#pragma region Validators
		DataValidator^ DataGeneratorSessantaquattroPiu::DefaultTriggerValidator::get()
		{
...
#pragma unmanaged
		...
		ULONG _stdcall AsyncIoSessantaquattroPiu(PVOID  ThreadParameter)
		{
